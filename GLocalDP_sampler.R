...
X.global = list(X1[-c(1), ], X2[-c(1,2), ], X3[-c(1, 2, 3), ]) 
...
X.local = list(X1[1, ], X2[c(1, 2), ], X3[c(1,2,3), ]) 
# Save local variables as a list of matrices across the populations
X.local.matrix = list(X1[1, , drop = FALSE], X2[c(1, 2), ], X3[c(1,2,3), ]) 
...
for(iter in 2:num_iter){
  ...
  for(t in 1:L){
    if(n.j.t[[1]][t] == 0){
      ...
      x.j.bar.local[[1]][[t]] = sum(X.local[[1]][t.samples[[1]][[iter - 1]] == t])/n.j.t[[1]][t]
    }
  }
  ...
  for(t in 1:L){
    if(n.j.t[[2]][t] == 0){
      ...
      x.j.bar.local[[2]][[t]] = rowSums(X.local[[2]][, t.samples[[2]][[iter - 1]] == t, drop = FALSE])/n.j.t[[2]][t]
    }
  }
  
  for(t in 1:L){
    if(n.j.t[[3]][t] == 0){
      x.j.bar.local[[3]][[t]] = rep(0, nrow(X.local[[3]]))
    }else{
  ...
  lambda.1.hat = 0; m.1.hat = 0
  
  for(t in 1:L){
    if(n.j.t[[1]][t] == 0){
      S.1.local[t] = 0
      Z.1.local[t] = 0
      alpha.1.hat[t] = alpha.j0 + 0.5 * n.j.t[[1]][t] 
      beta.1.hat[t] = beta.j0 + 0.5 * (S.1.local[t] + Z.1.local[t])
      lambda.1.hat[t] = lambda.local[1] + n.j.t[[1]][t]
      m.1.hat[t] = ((lambda.local[1] * m.j0[[1]]) + (n.j.t[[1]][t] * x.j.bar.local[[1]][[t]]))/lambda.1.hat[t]
    }else{
      S.1.local[t] = sum((X.local.matrix[[1]][, t.samples[[1]][[iter - 1]] == t] - x.j.bar.local[[1]][[t]])^2)
      Z.1.local[t] = ((lambda.local[1] * n.j.t[[1]][t])/(lambda.local[1] + n.j.t[[1]][t])) * sum((x.j.bar.local[[1]][[t]] - m.j0[[1]])^2)
      alpha.1.hat[t] = alpha.j0 + 0.5 * n.j.t[[1]][t] 
      beta.1.hat[t] = beta.j0 + 0.5 * (S.1.local[t] + Z.1.local[t])
      lambda.1.hat[t] = lambda.local[1] + n.j.t[[1]][t]
      m.1.hat[t] = ((lambda.local[1] * m.j0[[1]]) + (n.j.t[[1]][t] * x.j.bar.local[[1]][[t]]))/lambda.1.hat[t]
  ...
  S.2.local = matrix(0, nrow = nrow(X.local[[2]]), L)
  Z.2.local = matrix(0, nrow = nrow(X.local[[2]]), L)
  alpha.2.hat = matrix(0, nrow = nrow(X.local[[2]]), L); beta.2.hat =matrix(0, nrow = nrow(X.local[[2]]), L)
  lambda.2.hat = matrix(0, nrow = nrow(X.local[[2]]), L); m.2.hat = matrix(0, nrow = nrow(X.local[[2]]), L)
  
  for(t in 1:L){
    if(n.j.t[[2]][t] != 0){
      S.2.local[, t] = rowSums((X.local.matrix[[2]][, t.samples[[2]][[iter - 1]] == t, drop = FALSE] - x.j.bar.local[[2]][[t]])^2)
  ...
  S.3.local = matrix(0, nrow = nrow(X.local[[3]]), L)
  Z.3.local = matrix(0, nrow = nrow(X.local[[3]]), L)
  alpha.3.hat = matrix(0, nrow = nrow(X.local[[3]]), L); beta.3.hat =matrix(0, nrow = nrow(X.local[[3]]), L)
  lambda.3.hat = matrix(0, nrow = nrow(X.local[[3]]), L); m.3.hat = matrix(0, nrow = nrow(X.local[[3]]), L)
  
  for(t in 1:L){
    if(n.j.t[[3]][t] != 0){
      S.3.local[, t] = rowSums((X.local.matrix[[3]][, t.samples[[3]][[iter - 1]] == t, drop = FALSE] - x.j.bar.local[[3]][[t]])^2)
  ...
  m.j.samples <- list(matrix(0, nrow = nrow(X.local.matrix[[1]]), ncol = L),
                      ...
                      matrix(0, nrow = nrow(X.local.matrix[[3]]), ncol = L))
  ...
  for(t in 1:L){
    tau2.j.samples[[1]][, t] = rinvgamma(n = 1, alpha = alpha.1.hat[t], beta = beta.1.hat[t])
    m.j.samples[[1]][, t] = rnorm(n = 1, mean = m.1.hat[t], sd = sqrt(tau2.j.samples[[1]][,t] /lambda.1.hat[t]))
  }
  for(i in 1:nrow(alpha.2.hat)){
    tau2.j.samples[[2]][i, ] = rinvgamma(n = L, alpha = alpha.2.hat[i, ], beta = beta.2.hat[i, ])
    m.j.samples[[2]][i, ] = rnorm(n = L, mean = m.2.hat[i, ], sd = sqrt(tau2.j.samples[[2]][i,] /lambda.2.hat[i,]))
  }
  for(i in 1:nrow(alpha.3.hat)){
    tau2.j.samples[[3]][i, ] = rinvgamma(n = L, alpha = alpha.3.hat[i, ], beta = beta.3.hat[i, ])
    m.j.samples[[3]][i, ] = rnorm(n = L, mean = m.3.hat[i, ], sd = sqrt(tau2.j.samples[[3]][i,] /lambda.3.hat[i,]))
  }
  ...
  Tau2.j.samples.array3 = array(0, dim = c(nrow(X.local[[3]]), nrow(X.local[[3]]), L))
  # This is needed as the Rcpp function to calculate LL needs a cube for variance samples
  for(j in 1:ncol(Tau2.j.samples[[3]][[iter]])){
    Tau2.j.samples.array3[,,j] = diag(Tau2.j.samples[[3]][[iter]][, j], nrow = nrow(X.local[[3]]))
  }
  ...
  t.prob[[1]] <- prob_exponent_univ_local(pi_samples = pi.samples[[1]][[iter]], X_local = as.numeric(X.local[[1]]), psi_samples = as.numeric(m.j.samples[[1]]), Sigma = as.numeric(sqrt(tau2.j.samples[[1]])), 
  ...
  for(j in 2:J){
    t.prob[[j]] = prob_exponent_mv_local(pi_samples = pi.samples[[j]][[iter]], 
                                         X_local = t(X.local[[j]]), 
                                         psi_samples = matrix(M.j.samples[[j]][[iter]], nrow = nrow(X.local[[j]]), ncol = L),
...
for(iter in 2:num_iter){
  ...
  logsum = logll_univ_local2(pi.samples[[1]][[iter]], X.local[[1]], as.numeric(M.j.samples[[1]][[iter]]), 
                             Sigma = sqrt(as.numeric(Tau2.j.samples[[1]][[iter]])),
                             t.samples[[1]][[iter]], matrix(M.k.samples[[iter]], nrow = p.global, ncol = L),
                             X.global[[1]], k.samples[[1]][[iter]], 
                             Sigma_G = Tau2.k.samples.array) + 
    logll_mv_local2(pi.samples[[2]][[iter]], X.local[[2]],
                    matrix(M.j.samples[[2]][[iter]], nrow = 2, ncol = L),  
                    Sigma = Tau2.j.samples.array2,
                    t.samples[[2]][[iter]], 
                    matrix(M.k.samples[[iter]], nrow = p.global, ncol = L), 
                    X.global[[2]], k.samples[[2]][[iter]], 
                    Sigma_G = Tau2.k.samples.array) + 
    logll_mv_local2(pi.samples[[3]][[iter]], X.local[[3]],
                    matrix(M.j.samples[[3]][[iter]], nrow = 3, ncol = L),  
  ...
  for(l in 1:ncol(M.j.samples[[1]][[iter]])){
    Local_density1[l] <- dnorm(M.j.samples[[1]][[iter]][, l], mean = m.j0[[1]], sd = sqrt(1/(lambda.local[1]) * Tau2.j.samples[[1]][[iter]])[, l], log = TRUE)
  }
  Local_density1_sum = sum(Local_density1) + sum(dinvgamma(as.numeric(Tau2.j.samples[[1]][[iter]]), alpha = alpha0, beta = beta0, log = TRUE))
  
  Local_density2 = 0
  for(l in 1:ncol(M.j.samples[[2]][[iter]])){
    Local_density2[l] <- dmvnorm(M.j.samples[[2]][[iter]][, l], mean = m.j0[[2]], sigma = diag(1/(lambda.local[2]) * Tau2.j.samples[[2]][[iter]][, l], length(m.j0[[2]])), log = TRUE)
  }
  ...
  Local_density3 = 0
  for(l in 1:ncol(M.j.samples[[3]][[iter]])){
    Local_density3[l] <- dmvnorm(M.j.samples[[3]][[iter]][, l], mean = m.j0[[3]], sigma = diag(1/(lambda.local[3]) * Tau2.j.samples[[3]][[iter]][, l], length(m.j0[[3]])), log = TRUE)
  }
  
  Local_density3_sum = sum(Local_density3) + sum(dinvgamma(Tau2.j.samples[[3]][[iter]][1, ], alpha = alpha0, beta = beta0, log = TRUE)) + sum(dinvgamma(Tau2.j.samples[[3]][[iter]][2, ], alpha = alpha0, beta = beta0, log = TRUE)) + sum(dinvgamma(Tau2.j.samples[[3]][[iter]][3, ], alpha = alpha0, beta = beta0, log = TRUE))
  
  ## Component 1
  Local_density_sum = Local_density1_sum + Local_density2_sum + Local_density3_sum
...
################################################################################
x.limit.lower <- min(X1[2, ], X2[3, ], X3[4, ])
x.limit.upper <- max(X1[2, ], X2[3, ], X3[4, ])

y.limit.lower <- min(X1[3, ], X2[4, ], X3[5, ])
y.limit.upper <- max(X1[3, ], X2[4, ], X3[5, ])
